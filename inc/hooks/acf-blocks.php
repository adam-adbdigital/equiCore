...
{ // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
  return array_merge($categories, [
    [
      'slug'  => 'equicore',
      'title' => __('Theme blocks', 'equicore'),
    ],
  ]);
...
function acf_blocks_init()
{
  ...
  $example_data = apply_filters('equicore_acf_blocks_example_data', []);
  ...
  foreach (THEME_SETTINGS['acf_blocks'] as $block) {
    ...
    if (! isset($block['icon']) || empty($block['icon'])) {
      ...
      $icon_path = apply_filters('EquiCore_acf_block_icon', $icon_path, $block['name'], $block);
