  ...
  items.forEach((li) => {
    ...
    li.addEventListener('mouseover', function () {
      ...
      document.addEventListener('keydown', (keydownMouseoverEvent) => {
        // If rules don't apply, bail
        if (
          this.classList.contains('removing-hover') ||
          !this.classList.contains('hovering') ||
          !this.parentNode.classList.contains('hovering')
        ) {
          return;
        }
      ...
      setTimeout(() => {
        ...
        dropdownToggles.forEach((dropdownToggle) => {
          ...
          if (dropdownToggle.parentNode.querySelector('.dropdown')) {
            const linkText =
              dropdownToggle.parentNode.querySelector('.dropdown').textContent;

            // Set aria-label to expand for all dropdown-toggle elements
            // eslint-disable-next-line camelcase, no-undef
            dropdownToggle.setAttribute(
              'aria-label',
              `${EquiCore_screenReaderText.expand_for} ${linkText}`
            );
          }
        });
      }, hoverIntentTimeout);
